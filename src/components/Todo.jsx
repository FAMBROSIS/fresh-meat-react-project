...
const Todo = () => {
   ...
   const handleAddTodo=(taskIndex)=>{
    ...
    return (
        ...
        <input id="input" placeholder="Task Name" value={task} onChange={(e) => setTask(e.target.value)}/>
        ...
        <div>
          ...
          {todo.map((task, taskIndex) => (
            ...
            <div key={taskIndex}>
              ...
              <h3>{task.name}</h3>
              <button id='rmTaskButton' onClick={()=>{rmTask(taskIndex)}}>Clear</button>
              ...
              <ul>
                ...
                {task.items.map((todoItems, todoIndex) => (
                  ...
                  <li key={todoIndex}>
                    ...
                    {updateItem && updateItem.taskIndex === taskIndex && updateItem.todoIndex === todoIndex ? (
                      <div>
                      ...
                      <div>
                        {todoItems}
                        <button onClick={() => handleSelect(taskIndex, todoIndex)}>
                          Edit
                        </button>
                        <button onClick={() => handleClickRm(taskIndex, todoIndex)}>
                          X
                        </button>
                      </div>
                    )}
